...
def createTable (db_name: str, table_name: str, columns: dict) -> str:
    ...
    columns = {col_name.lower(): col_type.lower() for col_name, col_type in columns.items()}

    valid_types = ["int", "float", "complex", "str", "bool", "bytes", "list","datetime"]
    ...
    try:
        with open(table_path, 'w', newline='') as file:
            ...
            header = [f"{col_name}:{col_type}" for col_name, col_type in columns.items()]
...
if __name__ == "__main__":
    ...
    result = createDatabase(db_name)
    if(result) :
        print(result)
    else:
        print(f"Table created successfully at {db_name.lower()}")